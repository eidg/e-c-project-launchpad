...
const PatternDocDialog = ({ open, onClose, onSave, initialName = "", initialContent = "", title = "New Pattern Document", confirmLabel = "Save" }) => {
  ...
  const editor = useEditor({
    extensions: [
      StarterKit,
      Placeholder.configure({ placeholder: "Write or paste your pattern document..." }),
    ],
    content: initialContent,
    autofocus: true,
    editorProps: {
      attributes: {
        class: "prose max-w-none min-h-[220px] border border-gray-300 rounded-md p-3 focus:outline-none",
      },
    },
  });
      ...
      <DialogContent dividers>
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 border border-gray-300 rounded-md p-2 bg-gray-50">
            ...
            <button
              type="button"
              className={`px-2 py-1 rounded ${editor?.isActive('heading', { level: 2 }) ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-200'}`}
            ...
            <button
              type="button"
              className={`px-2 py-1 rounded ${editor?.isActive('heading', { level: 3 }) ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-200'}`}
...
export default PatternDocDialog;
